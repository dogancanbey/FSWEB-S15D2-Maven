...
public class StringSet {
    public static Set<String> findUniqueWords(){
        String text = "Carroll began writing the manuscript of the story the next day, although that earliest version is lost. " +
                ...
                "by Carroll, dedicating it as \"Christmas Gift to a Dear Child in Memory of a Summer's Day.\" " +
    ...
    String[] words = text.toLowerCase().replaceAll("[^a-zA-Z\\s]", " ").split("\\s");
        System.out.println("önce "+ words.length);
        TreeSet<String> uniqueWords = new TreeSet<>(Arrays.asList(words));
        System.out.println("sonra"+ uniqueWords.size());
        return uniqueWords;
    }
}
